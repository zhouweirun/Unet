...
def select_model(args, device):
    ...
    if model_id == 0:
        ...
        tile = None  # 根据你的需求设置分块大小
    elif model_id == 13:  # 添加你的模型
        from models.team13_unet import UNet
        name = "Unet"
        data_range = 1.0  # 根据训练时输入范围选择[0,1]或[0,255]
        model = UNet()
        model_path = "model_zoo/team13_Unet.pth"
        state_dict = torch.load(model_path)["model_state_dict"]
        model.load_state_dict(state_dict)
        tile = 256  # 分块大小（如不需要分块设为None）
    else:
